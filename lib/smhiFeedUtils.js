...
/**
 * This function normalizes warning levels for code-based comparisons.
 * @param {*} warningLevelCode - A warning level value from config or feed item.
 * @returns {string|undefined} - An uppercase warning level code, or undefined when blank.
 */
const normalizeWarningLevelCode = (warningLevelCode) => {
    const normalized = String(warningLevelCode || "").trim().toUpperCase();
    return normalized || undefined;
};

/**
 * This function checks whether an item should be included based on smhiShowWarningLevels.
 * @param {*} item - The formatted SMHI feed item.
 * @param {*} config - Configuration object containing smhiShowWarningLevels.
 * @returns {boolean} - True when warning level is allowed or no valid warning-level filter exists.
 */
const passesWarningLevelFilter = (item, config = {}) => {
    if (!Array.isArray(config.smhiShowWarningLevels) || config.smhiShowWarningLevels.length === 0) {
        return true;
    }

    const allowedLevels = config.smhiShowWarningLevels
        .map((levelCode) => normalizeWarningLevelCode(levelCode))
        .filter(Boolean);

    if (allowedLevels.length === 0) return true;

    const itemLevelCode = normalizeWarningLevelCode(item.warningLevelCode);
    if (!itemLevelCode) return false;

    return allowedLevels.includes(itemLevelCode);
};
...
const formatSMHIFeed = (feed, config = {}) => {
    ...
    return feed.flatMap(item => {
        ...
        return item.warningAreas.map(warning => {
            ...
            // Resolve the warning level text and code
            const warningLevelSource = warning.warningLevel || {};
            const resolvedWarningLevelText = resolveLocalizedValue(warningLevelSource, preferredLocale);
            const resolvedWarningLevelCode = normalizeWarningLevelCode(warningLevelSource.code);

            return {
                publishTime: warning.published,
                updatedTime: undefined,
                warningLevelCode: resolvedWarningLevelCode,
                warningLevelText: resolvedWarningLevelText,
...
function filterSMHIFeedItems(formattedFeed, excludeTimeFilter = false, configOrFilters = {}) {
    ...
    return formattedFeed.filter((formattedFeedItem) => {
        ...
        const warningLevelOk = passesWarningLevelFilter(formattedFeedItem, configOrFilters);
        return areaOk && contentOk && warningLevelOk;
    });
}
