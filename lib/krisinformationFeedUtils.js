...
function formatKrisinformationFeed(feed, config = {}) {
	...
	return feed.flatMap((item) => {
		...
		return {
			...
			startTime: item.Published,
			stopTime: getNewestTimeWithDaysAdded(item.Published, item.Updated, config.oldest),
...
function getNewestTimeWithDaysAdded(publishTime, updatedTime, daysToAdd) {
    if (!publishTime && !updatedTime) {
        return undefined;
    }

    const newestTime = publishTime > updatedTime ? publishTime : updatedTime;
    const newTime = new Date(newestTime);
    newTime.setDate(newTime.getDate() + daysToAdd);
    return newTime;
}
...
const isWarningRelevant = (formattedFeedItem) => {
    const now = new Date();
    if (formattedFeedItem.publishTime) {
        const startTime = new Date(formattedFeedItem.startTime);
        if (now < startTime) return false;
    }
    if (formattedFeedItem.stopTime) {
        const stopTime = new Date(formattedFeedItem.stopTime);
